...
import (
	...
	"github.com/EarthDollar/go-earthdollar/internal/build"
)

var (
	// Files that end up in the ged*.zip archive.
	gedArchiveFiles = []string{
		"COPYING",
		executablePath("ged"),
	}

	// Files that end up in the ged-alltools*.zip archive.
...
func doArchive(cmdline []string) {
	var (
		...
		upload = flag.String("upload", "", `Destination to upload the archives (usually "gedstore/builds")`)
		ext    string
	)
	flag.CommandLine.Parse(cmdline)
	switch *atype {
	...
	var (
		...
		ged     = "ged-" + base + ext
		alltools = "ged-alltools-" + base + ext
	)
	maybeSkipArchive(env)
	if err := build.WriteArchive(ged, gedArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
	for _, archive := range []string{ged, alltools} {
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "ged-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	installer, _ := filepath.Abs("ged-" + archiveBasename(*arch, env) + ".exe")
...
func doAndroidArchive(cmdline []string) {
	var (
		...
		upload = flag.String("upload", "", `Destination to upload the archive (usually "gedstore/builds")`)
	)
	flag.CommandLine.Parse(cmdline)
	env := build.Env()
	...
	if *local {
		...
		os.Rename("ged.aar", filepath.Join(GOBIN, "ged.aar"))
		return
	}
	...
	archive := "ged-" + archiveBasename("android", env) + ".aar"
	os.Rename("ged.aar", archive)
...
func doXCodeFramework(cmdline []string) {
	...
	build.MustRun(goTool("get", "golang.org/x/mobile/cmd/gomobile"))
	build.MustRun(gomobileTool("init"))
	bind := gomobileTool("bind", "--target", "ios", "--tags", "ios", "-v", "github.com/EarthDollar/go-earthdollar/mobile")
	...
	archive := "ged-" + archiveBasename("ios", env)
