...
func (s *simulation) launchNode(log bool) *Network {
	...
	ip[0] = 10
	addr := &net.UDPAddr{IP: ip, Port: 20203}
...
func (st *simTransport) sendPing(remote *Node, remoteAddr *net.UDPAddr, topics []Topic) []byte {
	...
	st.sendPacket(remote.ID, ingressPacket{
		...
		data: &ping{
			Version:    4,
			From:       rpcEndpoint{IP: st.senderAddr.IP, UDP: uint16(st.senderAddr.Port), TCP: 20203},
			To:         rpcEndpoint{IP: remoteAddr.IP, UDP: uint16(remoteAddr.Port), TCP: 20203},
...
func (st *simTransport) sendPong(remote *Node, pingHash []byte) {
	...
	st.sendPacket(remote.ID, ingressPacket{
		...
		ev:         pongPacket,
		data: &pong{
			To:         rpcEndpoint{IP: raddr.IP, UDP: uint16(raddr.Port), TCP: 20203},
			ReplyTok:   pingHash,
			Expiration: uint64(time.Now().Unix() + int64(expiration)),
		},
	})
