...
// Package xed is the interface to all Earthdollar functionality.
package xed
...
type XEd struct {
	...
	// read-only fields
	backend       *ed.Earthdollar
	frontend      Frontend
	agent         *miner.RemoteAgent
	gpo           *ed.GasPriceOracle
	state         *State
	whisper       *Whisper
	filterManager *filters.FilterSystem
}

func NewTest(ed *ed.Earthdollar, frontend Frontend) *XEd {
	return &XEd{backend: ed, frontend: frontend}
}

// New creates an XEd that uses the given frontend.
// If a nil Frontend is provided, a default frontend which
// confirms all transactions will be used.
func New(earthdollar *ed.Earthdollar, frontend Frontend) *XEd {
	xed := &XEd{
		backend:          earthdollar,
		frontend:         frontend,
		quit:             make(chan struct{}),
		filterManager:    filters.NewFilterSystem(earthdollar.EventMux()),
		logQueue:         make(map[int]*logQueue),
		blockQueue:       make(map[int]*hashQueue),
		transactionQueue: make(map[int]*hashQueue),
		messages:         make(map[int]*whisperFilter),
		agent:            miner.NewRemoteAgent(),
		gpo:              ed.NewGasPriceOracle(earthdollar),
	}
	if earthdollar.Whisper() != nil {
		xed.whisper = NewWhisper(earthdollar.Whisper())
	}
	earthdollar.Miner().Register(xed.agent)
	if frontend == nil {
		xed.frontend = dummyFrontend{}
	}
	state, _ := xed.backend.BlockChain().State()
	xed.state = NewState(xed, state)
	go xed.start()
	return xed
}
...
func (self *XEd) DefaultGasPrice() *big.Int {
	return self.gpo.SuggestPrice()
}

func (self *XEd) RemoteMining() *miner.RemoteAgent { return self.agent }

func (self *XEd) AtStateNum(num int64) *XEd {
...
func (self *XEd) WithState(statedb *state.StateDB) *XEd {
	xed := &XEd{
	...
	xed.state = NewState(xed, statedb)
	return xed
}

func (self *XEd) State() *State { return self.state }
...
func (self *XEd) UpdateState() (wait chan *big.Int) {
...
func (self *XEd) Whisper() *Whisper { return self.whisper }

func (self *XEd) getBlockByHeight(height int64) *types.Block {
	var num uint64
...
func (self *XEd) BlockByHash(strHash string) *Block {
...
func (self *XEd) EdBlockByHash(strHash string) *types.Block {
	hash := common.HexToHash(strHash)
	block := self.backend.BlockChain().GetBlock(hash)
...
func (self *XEd) EdTransactionByHash(hash string) (*types.Transaction, common.Hash, uint64, uint64) {
...
func (self *XEd) BlockByNumber(num int64) *Block {
	return NewBlock(self.getBlockByHeight(num))
}

func (self *XEd) EdBlockByNumber(num int64) *types.Block {
	return self.getBlockByHeight(num)
}

func (self *XEd) Td(hash common.Hash) *big.Int {
	return self.backend.BlockChain().GetTd(hash)
}

func (self *XEd) CurrentBlock() *types.Block {
	return self.backend.BlockChain().CurrentBlock()
}

func (self *XEd) GetBlockReceipts(bhash common.Hash) types.Receipts {
	return core.GetBlockReceipts(self.backend.ChainDb(), bhash)
}

func (self *XEd) GetTxReceipt(txhash common.Hash) *types.Receipt {
	return core.GetReceipt(self.backend.ChainDb(), txhash)
}

func (self *XEd) GasLimit() *big.Int {
	return self.backend.BlockChain().GasLimit()
}

func (self *XEd) Block(v interface{}) *Block {
	if n, ok := v.(int32); ok {
...
func (self *XEd) Solc() (*compiler.Solidity, error) {
	return self.backend.Solc()
}

// set in js console via admin interface or wrapper from cli flags
func (self *XEd) SetSolc(solcPath string) (*compiler.Solidity, error) {
	self.backend.SetSolc(solcPath)
	return self.Solc()
}

// store DApp value in extra database
func (self *XEd) DbPut(key, val []byte) bool {
	self.backend.DappDb().Put(append(dappStorePre, key...), val)
	return true
}

// retrieve DApp value from extra database
func (self *XEd) DbGet(key []byte) ([]byte, error) {
	val, err := self.backend.DappDb().Get(append(dappStorePre, key...))
	return val, err
}

func (self *XEd) PeerCount() int {
	return self.backend.PeerCount()
}

func (self *XEd) IsMining() bool {
	return self.backend.IsMining()
}

func (self *XEd) HashRate() int64 {
	return self.backend.Miner().HashRate()
}

func (self *XEd) EdVersion() string {
	return fmt.Sprintf("%d", self.backend.EdVersion())
}

func (self *XEd) NetworkVersion() string {
	return fmt.Sprintf("%d", self.backend.NetVersion())
}

func (self *XEd) WhisperVersion() string {
	return fmt.Sprintf("%d", self.backend.ShhVersion())
}

func (self *XEd) ClientVersion() string {
	return self.backend.ClientVersion()
}

func (self *XEd) SetMining(shouldmine bool, threads int) bool {
...
func (self *XEd) IsListening() bool {
	return self.backend.IsListening()
}

func (self *XEd) Coinbase() string {
	eb, err := self.backend.Earthbase()
	if err != nil {
		return "0x0"
	}
	return eb.Hex()
...
//earthdollar
func (self *XEd) MintAdd() string {
	return "Working Update xed.go"
}

func (self *XEd) NumberToHuman(balance string) string {
	b := common.Big(balance)

	return common.CurrencyToString(b)
}

func (self *XEd) StorageAt(addr, storageAddr string) string {
	return self.State().state.GetState(common.HexToAddress(addr), common.HexToHash(storageAddr)).Hex()
}

func (self *XEd) BalanceAt(addr string) string {
	return common.ToHex(self.State().state.GetBalance(common.HexToAddress(addr)).Bytes())
}

func (self *XEd) TxCountAt(address string) int {
	return int(self.State().state.GetNonce(common.HexToAddress(address)))
}

func (self *XEd) CodeAt(address string) string {
	return common.ToHex(self.State().state.GetCode(common.HexToAddress(address)))
}

func (self *XEd) CodeAtBytes(address string) []byte {
	return self.State().SafeGet(address).Code()
}

func (self *XEd) IsContract(address string) bool {
	return len(self.State().SafeGet(address).Code()) > 0
}

func (self *XEd) UninstallFilter(id int) bool {
...
func (self *XEd) NewLogFilter(earliest, latest int64, skip, max int, address []string, topics [][]string) int {
	self.logMu.Lock()
	defer self.logMu.Unlock()
...
func (self *XEd) NewTransactionFilter() int {
	self.transactionMu.Lock()
	defer self.transactionMu.Unlock()
...
func (self *XEd) NewBlockFilter() int {
	self.blockMu.Lock()
	defer self.blockMu.Unlock()
...
func (self *XEd) GetFilterType(id int) byte {
	if _, ok := self.blockQueue[id]; ok {
...
func (self *XEd) LogFilterChanged(id int) vm.Logs {
	self.logMu.Lock()
	defer self.logMu.Unlock()
...
func (self *XEd) BlockFilterChanged(id int) []common.Hash {
	self.blockMu.Lock()
	defer self.blockMu.Unlock()
...
func (self *XEd) TransactionFilterChanged(id int) []common.Hash {
	self.blockMu.Lock()
	defer self.blockMu.Unlock()
...
func (self *XEd) Logs(id int) vm.Logs {
...
func (self *XEd) AllLogs(earliest, latest int64, skip, max int, address []string, topics [][]string) vm.Logs {
	filter := filters.New(self.backend.ChainDb())
	filter.SetBeginBlock(earliest)
	filter.SetEndBlock(latest)
	filter.SetAddresses(cAddress(address))
...
// HEX encoded.
func (p *XEd) NewWhisperFilter(to, from string, topics [][]string) int {
	// Pre-define the id to be filled later
	var id int

	// Callback to delegate core whisper messages to this xed filter
	...
	// Initialize the core whisper filter and wrap into xed
...
func (p *XEd) UninstallWhisperFilter(id int) bool {
	p.messagesMu.Lock()
...
func (self *XEd) WhisperMessages(id int) []WhisperMessage {
...
func (self *XEd) WhisperMessagesChanged(id int) []WhisperMessage {
	self.messagesMu.RLock()
	defer self.messagesMu.RUnlock()
...
// func (self *XEd) Register(args string) bool {
// 	self.regmut.Lock()
...
// func (self *XEd) Unregister(args string) bool {
// 	self.regmut.Lock()
// 	defer self.regmut.Unlock()
...
// func (self *XEd) PullWatchTx(args string) []*interface{} {
// 	self.regmut.Lock()
// 	defer self.regmut.Unlock()
...
func (self *XEd) EachStorage(addr string) string {
	var values []KeyVal
...
func (self *XEd) ToAscii(str string) string {
	padded := common.RightPadBytes([]byte(str), 32)

	return "0x" + common.ToHex(padded)
}

func (self *XEd) FromAscii(str string) string {
	if common.IsHex(str) {
		str = str[2:]
	}
...
func (self *XEd) FromNumber(str string) string {
	if common.IsHex(str) {
		str = str[2:]
	}
...
func (self *XEd) PushTx(encodedTx string) (string, error) {
...
func (self *XEd) Call(fromStr, toStr, valueStr, gasStr, gasPriceStr, dataStr string) (string, string, error) {
...
func (self *XEd) ConfirmTransaction(tx string) bool {
	return self.frontend.ConfirmTransaction(tx)
}

func (self *XEd) doSign(from common.Address, hash common.Hash, didUnlock bool) ([]byte, error) {
...
func (self *XEd) Sign(fromStr, hashStr string, didUnlock bool) (string, error) {
	var (
...
func (self *XEd) Frontend() Frontend {
	return self.frontend
}

func (self *XEd) SignTransaction(fromStr, toStr, nonceStr, valueStr, gasStr, gasPriceStr, codeStr string) (*types.Transaction, error) {
...
func (self *XEd) Transact(fromStr, toStr, nonceStr, valueStr, gasStr, gasPriceStr, codeStr string) (string, error) {
...
func (self *XEd) sign(tx *types.Transaction, from common.Address, didUnlock bool) (*types.Transaction, error) {
	hash := tx.SigHash()
	sig, err := self.doSign(from, hash, didUnlock)
	if err != nil {
		return tx, err
