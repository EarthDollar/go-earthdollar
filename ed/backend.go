// Copyright 2014 The go-edereum Authors
...
type Ethereum struct {
	...
	shutdownChan  chan bool // Channel for shutting down the edereum
...
func New(ctx *node.ServiceContext, config *Config) (*Ethereum, error) {
	...
	glog.V(logger.Info).Infoln("Chain config:", ed.chainConfig)

	ed.blockchain, err = core.NewBlockChain(chainDb, ed.chainConfig, ed.pow, ed.EventMux(), vm.Config{EnablePreimageRecording: config.EnablePreimageRecording})
	if err != nil {
	...
	if ed.protocolManager, err = NewProtocolManager(ed.chainConfig, config.FastSync, config.NetworkId, maxPeers, ed.eventMux, ed.txPool, ed.pow, ed.blockchain, chainDb); err != nil {
		return nil, err
	}
	...
	gpo := gasprice.NewGasPriceOracle(ed.blockchain, chainDb, ed.eventMux, gpoParams)
	ed.ApiBackend = &EthApiBackend{ed, gpo}

	return ed, nil
}

// CreateDB creates the chain database.
func CreateDB(ctx *node.ServiceContext, config *Config, name string) (eddb.Database, error) {
...
func SetupGenesisBlock(chainDb *eddb.Database, config *Config) error {
...
func CreatePoW(config *Config) (pow.PoW, error) {
	switch {
	case config.PowFake:
		glog.V(logger.Info).Infof("edash used in fake mode")
		return pow.PoW(core.FakePow{}), nil
	case config.PowTest:
		glog.V(logger.Info).Infof("edash used in test mode")
		return edash.NewForTesting()
	case config.PowShared:
		glog.V(logger.Info).Infof("edash used in shared mode")
		return edash.NewShared(), nil
	default:
		return edash.New(), nil
	}
}
...
func (s *Ethereum) StartMining(threads int) error {
	eb, err := s.Etherbase()
	if err != nil {
		err = fmt.Errorf("Cannot start mining without ederbase address: %v", err)
...
func (s *Ethereum) Start(srvr *p2p.Server) error {
	s.netRPCService = edapi.NewPublicNetAPI(srvr, s.NetVersion())
	if s.AutoDAG {
		s.StartAutoDAG()
	}
...
// it calls edash.MakeDAG  to pregenerate the DAG for the next epoch n+1
...
func (self *Ethereum) StartAutoDAG() {
	...
	go func() {
		glog.V(logger.Info).Infof("Automatic pregeneration of edash DAG ON (edash dir: %s)", edash.DefaultDir)
		...
		for {
			select {
			case <-timer:
				glog.V(logger.Info).Infof("checking DAG (edash dir: %s)", edash.DefaultDir)
				...
				if nextEpoch <= thisEpoch {
					if currentBlock%epochLength > autoDAGepochHeight {
						if thisEpoch > 0 {
							...
							os.Remove(filepath.Join(edash.DefaultDir, previousDag))
							os.Remove(filepath.Join(edash.DefaultDir, previousDagFull))
						...
						if _, err := os.Stat(dag); os.IsNotExist(err) {
							...
							err := edash.MakeDAG(nextEpoch*epochLength, "") // "" -> edash.DefaultDir
							if err != nil {
...
func (self *Ethereum) StopAutoDAG() {
	...
	glog.V(logger.Info).Infof("Automatic pregeneration of edash DAG OFF (edash dir: %s)", edash.DefaultDir)
}

// dagFiles(epoch) returns the two alternative DAG filenames (not a path)
// 1) <revision>-<hex(seedhash[8])> 2) full-R<revision>-<hex(seedhash[8])>
func dagFiles(epoch uint64) (string, string) {
	seedHash, _ := edash.GetSeedHash(epoch * epochLength)
	dag := fmt.Sprintf("full-R%d-%x", edashRevision, seedHash[:8])
