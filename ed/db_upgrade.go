...
// been safely stopped.
func upgradeSequentialKeys(db eddb.Database) (stopFn func()) {
...
func upgradeSequentialCanonicalNumbers(db eddb.Database, stopFn func() bool) (error, bool) {
	prefix := []byte("block-num-")
	it := db.(*eddb.LDBDatabase).NewIterator()
	defer func() {
		it.Release()
	}()
	it.Seek(prefix)
...
// if successful.
func upgradeSequentialBlocks(db eddb.Database, stopFn func() bool) (error, bool) {
	prefix := []byte("block-")
	it := db.(*eddb.LDBDatabase).NewIterator()
	defer func() {
		it.Release()
...
func upgradeSequentialOrphanedReceipts(db eddb.Database, stopFn func() bool) (error, bool) {
...
func upgradeSequentialBlockData(db eddb.Database, hash []byte) error {
