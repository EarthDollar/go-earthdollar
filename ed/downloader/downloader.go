...
var (
	...
	hashTTL        = 3 * time.Second     // [ed/61] Time it takes for a hash request to time out
	blockTargetRTT = 3 * time.Second / 2 // [ed/61] Target time for completing a block retrieval request
	blockTTL       = 3 * blockTargetRTT  // [ed/61] Maximum time allowance before a block request is considered expired

	headerTTL        = 3 * time.Second      // [ed/62] Time it takes for a header request to time out
	bodyTargetRTT    = 3 * time.Second / 2  // [ed/62] Target time for completing a block body retrieval request
	bodyTTL          = 3 * bodyTargetRTT    // [ed/62] Maximum time allowance before a block body request is considered expired
	receiptTargetRTT = 3 * time.Second / 2  // [ed/63] Target time for completing a receipt retrieval request
	receiptTTL       = 3 * receiptTargetRTT // [ed/63] Maximum time allowance before a receipt request is considered expired
	stateTargetRTT   = 2 * time.Second / 2  // [ed/63] Target time for completing a state trie retrieval request
	stateTTL         = 3 * stateTargetRTT   // [ed/63] Maximum time allowance before a node data request is considered expired

	maxQueuedHashes   = 256 * 1024 // [ed/61] Maximum number of hashes to queue for import (DOS protection)
	maxQueuedHeaders  = 256 * 1024 // [ed/62] Maximum number of headers to queue for import (DOS protection)
	maxQueuedStates   = 256 * 1024 // [ed/63] Maximum number of state requests to queue (DOS protection)
...
type Downloader struct {
	...
	// Channels
	newPeerCh     chan *peer
	hashCh        chan dataPack // [ed/61] Channel receiving inbound hashes
	blockCh       chan dataPack // [ed/61] Channel receiving inbound blocks
	headerCh      chan dataPack // [ed/62] Channel receiving inbound block headers
	bodyCh        chan dataPack // [ed/62] Channel receiving inbound block bodies
	...
	blockWakeCh   chan bool     // [ed/61] Channel to signal the block fetcher of new tasks
	bodyWakeCh    chan bool     // [ed/62] Channel to signal the block body fetcher of new tasks
...
func New(stateDb eddb.Database, mux *event.TypeMux, hasHeader headerCheckFn, hasBlockAndState blockAndStateCheckFn,
...
func (d *Downloader) syncWithPeer(p *peer, hash common.Hash, td *big.Int) (err error) {
	...
	glog.V(logger.Debug).Infof("Synchronising with the network using: %s [ed/%d]", p.id, p.version)
...
func (d *Downloader) fetchHeight61(p *peer) (uint64, error) {
	...
	for {
		...
		case <-d.stateCh:
		case <-d.receiptCh:
			// Ignore ed/{62,63} packets because this is ed/61.
...
func (d *Downloader) fetchHeight(p *peer) (uint64, error) {
	...
	for {
		...
		case <-d.hashCh:
		case <-d.blockCh:
			// Ignore ed/61 packets because this is ed/62+.
			// These can arrive as a late delivery from a previous sync.
		}
	}
}
