...
type BlockTraceResult struct {
	...
	StructLogs []edapi.StructLogRes `json:"structLogs"`
...
func (api *PrivateDebugAPI) TraceBlock(blockRlp []byte, config *vm.LogConfig) BlockTraceResult {
	...
	return BlockTraceResult{
		Validated:  validated,
		StructLogs: edapi.FormatLogs(logs),
...
func (api *PrivateDebugAPI) TraceTransaction(ctx context.Context, txHash common.Hash, config *TraceArgs) (interface{}, error) {
	...
	if config != nil && config.Tracer != nil {
		...
		var err error
		if tracer, err = edapi.NewJavascriptTracer(*config.Tracer); err != nil {
			return nil, err
		}
		...
		go func() {
			<-deadlineCtx.Done()
			tracer.(*edapi.JavascriptTracer).Stop(&timeoutError{})
		}()
		defer cancel()
	} else if config == nil {
	...
	for idx, tx := range block.Transactions() {
		...
		case *vm.StructLogger:
			return &edapi.ExecutionResult{
				Gas:         gas,
				ReturnValue: fmt.Sprintf("%x", ret),
				StructLogs:  edapi.FormatLogs(tracer.StructLogs()),
			}, nil
		case *edapi.JavascriptTracer:
