...
func tmpdir(t *testing.T) string {
	dir, err := ioutil.TempDir("", "ged-test")
	if err != nil {
		t.Fatal(err)
	}
	return dir
...
func runGed(t *testing.T, args ...string) *testged {
	...
	// Boot "ged". This actually runs the test binary but the init function
...
//
//     ged.expect(`Passphrase: {{.InputLine "password"}}`)
func (tt *testged) InputLine(s string) string {
	io.WriteString(tt.stdin, s+"\n")
	return ""
}

func (tt *testged) setTemplateFunc(name string, fn interface{}) {
	if tt.Func == nil {
...
// before matching.
func (tt *testged) expect(tplsource string) {
...
func (tt *testged) matchExactOutput(want []byte) error {
...
// after expectRegexp.
func (tt *testged) expectRegexp(resource string) (*regexp.Regexp, []string) {
	var (
...
// returned.
func (tt *testged) stderrText() string {
	tt.stderr.mu.Lock()
	defer tt.stderr.mu.Unlock()
	return tt.stderr.buf.String()
}

func (tt *testged) withKillTimeout(fn func()) {
