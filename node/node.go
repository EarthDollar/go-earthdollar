...
func (n *Node) OpenDatabase(name string, cache, handles int) (eddb.Database, error) {
	if n.config.DataDir == "" {
		return eddb.NewMemDatabase()
	}
	return eddb.NewLDBDatabase(n.config.resolvePath(name), cache, handles)
}
