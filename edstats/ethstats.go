...
type Service struct {
	...
	ed    *ed.Ethereum      // Full Ethereum service if monitoring a full node
...
func New(url string, edServ *ed.Ethereum, lesServ *les.LightEthereum) (*Service, error) {
...
func (s *Service) login(in *json.Decoder, out *json.Encoder) error {
	...
	if info := infos.Protocols["ed"]; info != nil {
		network = strconv.Itoa(info.(*ed.EthNodeInfo).Network)
		protocol = fmt.Sprintf("ed/%d", ed.ProtocolVersions[0])
	} else {
		network = strconv.Itoa(infos.Protocols["les"].(*ed.EthNodeInfo).Network)
...
func (s *Service) reportLatency(out *json.Encoder) error {
	// Send the current time to the edstats server
	start := time.Now()
...
func (s *Service) assembleBlockStats(block *types.Block) *blockStats {
	...
	if s.ed != nil {
		...
		if block == nil {
			block = s.ed.BlockChain().CurrentBlock()
		}
		header = block.Header()
		td = s.ed.BlockChain().GetTd(header.Hash(), header.Number.Uint64())
...
func (s *Service) reportHistory(out *json.Encoder, list []uint64) error {
	...
	if len(list) > 0 {
		...
		if s.ed != nil {
			head = s.ed.BlockChain().CurrentHeader()
		} else {
	...
	for i, number := range indexes {
		if s.ed != nil {
			history[i] = s.assembleBlockStats(s.ed.BlockChain().GetBlockByNumber(number))
		} else {
...
func (s *Service) reportStats(out *json.Encoder) error {
	...
	if s.ed != nil {
		mining = s.ed.Miner().Mining()
		hashrate = int(s.ed.Miner().HashRate())

		sync := s.ed.Downloader().Progress()
		syncing = s.ed.BlockChain().CurrentHeader().Number.Uint64() >= sync.HighestBlock

		gasprice = int(s.ed.Miner().GasPrice().Uint64())
	} else {
